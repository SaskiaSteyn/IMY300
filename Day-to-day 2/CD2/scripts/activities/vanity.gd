...
@onready var making_bed = preload("res://assets/sfx/shower.wav")
...
var rewards_func_day_zero = func(): 
	Global.change_energy.emit(10)
	var pop_up_scene = load("res://scenes/popups/showered_reward.tscn").instantiate()
...
func _on_mouse_entered() -> void:
	if(is_player_in_room and !has_showered):
...
func _on_input_event(viewport: Node, event: InputEvent, shape_idx: int) -> void:
	if event is InputEventMouseButton:
		if (event.pressed and event.button_index == MOUSE_BUTTON_LEFT and is_player_in_room and !Global.is_walking and !has_showered and is_activity_doable):
